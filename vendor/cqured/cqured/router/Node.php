...
class Node
{
    ...
    public function router($config)
    {
        ...
        $presentationApi = explode('/', $_GET['CquredQueryUrl'] ?? '/')[0];
        if (!is_null($presentationApi)) {

            for ($i = 0; $i < count($this->_appConfig->presentationApis); $i++) {
                if (strtolower($presentationApi) == strtolower($this->_appConfig->presentationApis[$i])) {
                    $this->_api = $presentationApi;
                    $this->_apiRouterPath = 'src' . DS . 'presentation' . DS . $this->_api . DS . 'Router.php';

                    break;
                }
            }
        } else {
            die('No Presentation Api Found');
        }

        if (isset($this->_apiRouterPath) && file_exists($this->_apiRouterPath)) {
            // echo $this->_apiRouterPath;
            $this->_appRoute();
        } else {
            echo 'The  Router file was not found at the specified destination <br><h2>Check the routerPath variable in config.php';
        }
    }
    ...
    private function _appRoute()
    {
        ...
        include_once $this->_apiRouterPath;
        ...
        $routerPath = $coreRouter
            ->getPath(
                str_replace($this->_api . '/', '', $url)
            );
        ...
        if ($routerPath !== null) {
            ...
            if (isset($routerPath['authguard'])) {
                ...
                for ($i = 0; $i < count($routerPath['authguard']); $i++) {
                    ...
                    $modelClass = '\\Presentation\\' . ucfirst($this->_api) . '\\Models\\' . $model;
            ...
            $controllerNamespace = $routerPath['namespace'] ??
            '\\Presentation\\' . ucfirst($this->_api) . '\\Controllers\\';
